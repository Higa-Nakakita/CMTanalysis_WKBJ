...
ローカルモードの固有パラメータで異常値（極端なQや群速度）を検出した場合は，PREMの値に置き換える処理が行われる．その記録はremedy_eigps.txtに出力される．
...
波線項の計算を行う．モードブランチ数，周波数index，周波数，観測点indexなどを受け取る．各モード，周波数について，ローカルモードを波線上で積算し，水平不均質性を反映させる．
