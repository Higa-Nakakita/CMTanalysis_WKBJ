...
pwddir = os.getcwd()
overlap_fil = "overlap.txt"
if os.path.exists(overlap_fil):
    os.remove(overlap_fil)
    ...
    for i in range(num_cross):
        ...
        for j in range(num_cross):
            ...
            if lat_ref==lat and lon_ref==lon :
                 k = k + 1
                 if k== 2 :
                    print('Detect overlap at',sta,lat,lon)
                    with open(overlap_fil,'a') as overlap_f:
                        #overlap_f.write(f'{pwddir} {slat} {slon}\n')  
                        overlap_f.write(f'{sta} {lat} {lon}\n')  
                    #print(k)
                    #print('stop??')
                    #sys.exit()
