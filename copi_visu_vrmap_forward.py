...
###########
df_rcv = pd.read_csv('rcv_coordinates',names=['sta','lat','lon'],delim_whitespace=True)
...
for iw in range(nwave):         # 行: 時間窓
    for icomp, comp in enumerate(components):  # 列: 成分
        ...
        ax.set_title(f"{comp}, win={iw+1}, meanVR={VR_mn_value:.1f}%", fontsize=15)

# 共通カラーバーを右側に配置
cbar = fig.colorbar(sc, ax=axes.ravel().tolist(), orientation='vertical', fraction=0.02, pad=0.02)
cbar.set_label("VR (%)")

fig.suptitle("VR maps for {strucnm}", fontsize=16)
plt.tight_layout(rect=[0, 0, 0.9, 0.96])  # 右にカラーバー、上にタイトル

plt.savefig("VR_maps_all_windows.png", dpi=300)
plt.show()
