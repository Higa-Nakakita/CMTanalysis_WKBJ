...
def visu_waves():
    ...
    tmp = '/home/higa/Laboratory/Dec_2025/amp_prob/wkbj/WKBJ_1204_insta/Waveforms/' 
            ...
            for row in range(plots_perPage):
                ...
                for col in range(cmpnm):
                    ...
                    if OptorFor == 0:
                        ...
                        syn_path = syn_dirpath + f'syn_{sta}_tw{twnum}.{comp}.sac'
                        syn_path_ins = tmp + f'obs_{sta}_tw{twnum}.{comp}.sac' 
                    ...
                    syn_ins_st = read(syn_path_ins)

                    obs_time = obs_st[0].times()
                    obs_data = obs_st[0].data
                    syn_time = syn_st[0].times()
                    syn_data = syn_st[0].data
                    syn_ins_time = syn_ins_st[0].times()
                    syn_ins_data = syn_ins_st[0].data

                    axes[row][col].plot(obs_time,obs_data,color='k',linewidth=1.3,label='mineos')
                    axes[row][col].plot(syn_ins_time,syn_ins_data,color='b',linewidth=1,label='istaseis')
                    axes[row][col].plot(syn_time,syn_data,color='r',linewidth=1,linestyle='--',label='wkbj')
