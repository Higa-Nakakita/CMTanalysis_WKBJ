...
c						arrays used by NA routines
	real*4		misfit(nmod_max),
     ...
     &                  mfitmean,mfitminc
CKY
	real*4          misfitWAV(nmod_max),misfitvalWAV
CKY	common  /NA_misfitWAV/misfitWAV
...
CKY	      call forward(nd, jj, na_model, misfitval)
	      call forward(nd, jj, na_model, misfitval, misfitvalWAV)

	      misfit(jj) = misfitval
	      misfitWAV(jj) = misfitvalWAV
c
 30	   continue
c           t1 = cputime(t2,t3)
c           ttfor = ttfor + t2
 31        continue
CC	   if(summary)write(*,*)' finished forward modeling'
c
...
CC	call writemodels 
CC     &       (nd, ntot, na_models, misfit,
CC     &        nsamplei, nsample, itmax, nh_max, nh, header)
	call writemodels 
     &       (nd, ntot, na_models, misfit, misfitWAV,
...
c
	cnad='na.nad'
	call NA_header
     &       (lu_nad,cnad,header,nh_max,nh,nd,
     &        range,scales,
     &        nsamplei,nsample,ncells,nh_user)

	if(summary) then
           write(lu_out,*)' Total length of header in (nad) direct ',
     &               ' access model file = ',nh
           write(lu_out,*)
           write(lu_out,*)' Writing direct access file...'
           write(lu_out,*)
	end if
c                                               write direct access nad file
CKY        call write_nad
CKY     &       (lu_nad,cnad,nd,ntot,
CKY     &        nh,nh_user,header,na_models,misfit)
        call write_nad
     &       (lu_nad,cnad,nd,ntot,
     &        nh,nh_user,header,na_models,misfit,misfitWAV)
...
c
     ...
     &           (lu,fnme,nd,ne,nh,nhu,header,models,data,data2)
c
      real*4            models(nd,ne)
      real*4            data(ne)
      character         header(nh)
...
c						open direct access nad file
     ...
     &       form='unformatted',access='direct',recl=len)
      ...
      tail = -999.0
      write(lu,rec=1)nd,ne,nh,nhu,header,models,data,data2,tail
