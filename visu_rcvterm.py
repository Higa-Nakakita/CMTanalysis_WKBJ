...
def visu_fixterm():
    ...
    for idx, st_comp in enumerate(st_comps):
        ...
        for n in n_values:
            ...
            # Plot Re and Im
            axes[0, compnum].plot(df_n['frqtmp'], df_n['Re'], label=f'Re (n={n})', marker='o',ms=1.5 ,linewidth=0)
            axes[1, compnum].plot(df_n['frqtmp'], df_n['Im'], label=f'Im (n={n})', marker='o',ms=1.5 ,linewidth=0)
        
        # Set titles and labels
        axes[0, compnum].set_title(f'comp={st_comp}')
        axes[0, compnum].set_xlabel('Frequency (frqtmp)')
        axes[0, compnum].set_ylabel('Re zAexprcv')
        axes[0, compnum].legend(fontsize='small',loc='upper left', bbox_to_anchor=(1, 1))
        axes[1, compnum].set_title(f'comp={st_comp}')
        axes[1, compnum].set_xlabel('Frequency (frqtmp)')
        axes[1, compnum].set_ylabel('Im zAexprcv')
